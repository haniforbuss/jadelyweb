...
import { getStockData, getStockStatus } from '../lib/stockStorage';
import type { StockItem } from '../lib/stockStorage';
...
const Products = ({ onAddToCart }: ProductsProps) => {
  ...
  const [stockData, setStockDataState] = useState<Record<number, StockItem>>({});

  // Load stock from localStorage (set by admin)
  useEffect(() => {
    const data = getStockData();
    const init: Record<number, StockItem> = {};
    productsConfig.products.forEach((p) => {
      init[p.id] = data[p.id] ?? { stock: 50, minStock: 5 };
    });
    setStockDataState(init);
  }, []);
  ...
  const handleAddToCart = (product: Product) => {
    const s = stockData[product.id];
    if (s && getStockStatus(s.stock, s.minStock) === 'out-of-stock') return;
    onAddToCart(product);
            ...
            >
              ...
              <div className="relative h-[400px] overflow-hidden bg-[#fafafa]">
                ...
                {(() => {
                  const s = stockData[product.id];
                  if (!s) return null;
                  const status = getStockStatus(s.stock, s.minStock);
                ...
                {(() => {
                  const s = stockData[product.id];
                  const isOut = s && getStockStatus(s.stock, s.minStock) === 'out-of-stock';
                  return (
                    <button
                      ...
                      className={`absolute bottom-4 left-1/2 -translate-x-1/2 px-6 py-3 flex items-center gap-2 text-sm tracking-wide transition-all duration-300 ${
                        isOut
                          ? 'bg-slate-400 text-white opacity-80 cursor-not-allowed translate-y-0 opacity-100'
                    ...
                    >
                      {isOut ? (
                        ...
                        <><Check size={16} />{productsConfig.addedToCartText}</>
                      ) : (
                        <><ShoppingBag size={16} />{productsConfig.addToCartText}</>
                      )}
                    </button>
                  );
                })()}
